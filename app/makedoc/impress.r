REBOL [
	Title: "Impress Presentation Emitter"
	Type: 'emitter
]
...
emit-code: func [code [string!]][
	either parse/all code [":rebol^/" code: to end][
		require %markup/color-code-inline.r
		emit any [
			attempt [color-code code]
			[<pre><code> sanitize code </code></pre>]
		]
	][
		emit [<pre><code> sanitize code </code></pre>]
	]
]
...
emit-square: use [rule counter finish code mark][
	rule: [
		(code: none counter: 0 finish: [])
		some [
			  <sb> (++ counter <sb> counter)
			| </sb> (-- counter if 0 = counter [finish: [any skip]]) finish
			| mark: paren! (-- counter if 0 = counter [code: mark/1 finish: [any skip]]) finish
			| skip
		]
	]
	...
	has [value][
		...
		unless all [
			...
			parse code [
				...
				| set value ['b | 'i | 'strong | 'em | 'span | 'sup] end (
					emit value: to tag! form value
					hold head insert value "/"
				)
				| copy value [['b | 'i | 'strong | 'em | 'span | 'sup] word!] end (
					emit join to tag! value/1 [{ class="} sanitize form value/2 {"}]
					hold join </> value/1
				)
			]
		][
...
emit-video: func [spec [block!] /youtube /vimeo /local video][
	...
	either all [
		...
		any [issue? video/id video/id: get-video-id video/id]
	][
		video/id: join case [
			youtube [https://youtube.com/embed/]
			vimeo [http://player.vimeo.com/video/]
		] sanitize to string! video/id
		emit [{<div class="tube">^/<iframe type="text/html" src="} video/id {"></iframe>^/</div>}]
	][
		raise ["[INVALID VIDEO " sanitize form spec "]"]
	]
]
...
get-list-options: func [options [block!]][
	either options: match options [
		...
		class: opt 'incremental
	][
		make options [
			end: either tag = 'bullets [
				tag: to tag! rejoin ["ul" to-attr class]
				</ul>
			][
...
emit-step: use [attrs form-attr][
	...
	form-attr: func [word [word!] value][
		attrs/(word): either integer? value [
			rejoin [{ data-} form word {="} value {"}]
		][""]
	]

	emit-step: func [spec /local value][
		spec: copy spec
		set attrs ""
		attrs/class: copy ["step"]
		...
		attrs/class: some/empty attrs/class [
			rejoin [{ class="} sanitize reform attrs/class {"}]
		][""]

		emit rejoin compose [{<div} (values-of attrs) {>}]
	]
]
...
initial: [
	...
	para: (
		feed emit <div id="impress">
		feed emit <div id="intro" class="step slide" data-x="-2000" data-y="-1500">
		feed emit <div class="logo">
		emit <img src="http://reb4.me/x/forklogo-20130615-174319.png" alt="Rebol" width="120" height="100" title="Rebol" />
		emit </div>
		feed emit <h1> emit-inline data emit </h1>
	) boilerplate (feed emit </div>)
]
...
boilerplate: [
	...
	sect3: (feed emit <h3> emit-inline data emit </h3>)
	sect4: (feed emit <h4> emit-inline data emit </h4>)
	default: (feed emit </div>) continue steps
]

steps: [
	step: (feed feed emit-step data) normal (feed emit </div>)
]

normal: [
	step: continue return
	quote: (feed emit <q> emit-inline data emit </q>)
	para: (feed emit <p> emit-inline data emit </p>)
	nopara: (feed emit-inline data)
	...
	list-in:
		(
			...
			feed emit options/tag
			hold options
		)
		in-list
		(
			options: release
			feed emit options/end
		)
	center-in:
		(feed emit <center>)
		in-center
...
in-list: inherit in-block [
	para: (feed emit <li> emit-inline data emit </li>)
	bullet: bullet2: bullet3: (feed emit [<li> <ul> newline <li>] emit-inline data) in-bul (emit [</li> newline </ul> </li>])
	enum: enum2: enum3: (feed emit [<li> <ol> newline <li>] emit-inline data) in-enum (emit [</li> newline </ol> </li>])
	list-out: return
]

in-slide: inherit normal [
...
paragraph: [
	...
	<sup> (emit <sup>) in-sup (emit </sup>)
	<cite> (emit <cite>) in-cite (emit </cite>)
	<span> (emit <span>) in-span (emit </span>)
	<strong> (emit <strong>) in-strong (emit </strong>)
	...
	:refinement! (hold value) in-class
	</> ()
	default: (emit "[???]")
]
...
to-class: func [tag [tag!] class [refinement!]][
	join tag [{ class="} to string! class {"}]
]

in-class: [
	<b> (class: release emit to-class value class) in-bold (emit </b>)
	<i> (class: release emit to-class value class) in-italic (emit </i>)
	<span> (class: release emit to-class value class) in-span (emit </span>)
	<strong> (class: release emit to-class value class) in-strong (emit </strong>)
	default: continue return
]
...
in-sup: inherit paragraph [</sup> return </> continue return]

in-cite: inherit paragraph [</cite> return </> continue return]

in-span: inherit paragraph [</span> return </> continue return]

in-strong: inherit paragraph [</strong> return </> continue return]
