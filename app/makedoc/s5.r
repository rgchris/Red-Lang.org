...
initial: [
	...
	para:
		(
			feed emit <ol class="xoxo presentation">
			feed emit <li class="slide"> feed emit <h1> emit-inline data emit </h1>
		)
		boilerplate
		(emit </ol>)
	default: (emit "You didn't do this right")
]
...
boilerplate: [
	...
	default: (feed emit </li> feed) continue normal
]
