...
flickr: context bind [
	do: func [
		...
		method "HTTP method."
		api-method "Flickr API Method"
		params [block! none!] "Parameters to include with API Call"
		/auth "Require Authentication (to follow)"
	][
		engage method compose [method: (api-method) (any [params []])]
	]

	get: func [method params][do 'get method params]
	post: func [method params][do 'post method params]

	echo: func ["Echo" [catch]][
		get "flickr.test.echo" [echo: true engage: "REBOL"]
	]

	null: func ["Null" [catch]][
		get "flickr.test.null" none
	]
	...
	engage: func [method [word!] params [none! block!] /local result][
		all [
			find [get post] method
			params: send :method params
			result: load-json params

			switch result/stat [
				"ok" [result]
				"fail" [raise result/message]
			]
		]
	]
